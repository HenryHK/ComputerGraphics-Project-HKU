...
public:
	...
	Colour& operator =(const Colour& other); 
	Colour operator *(const Colour& other); 
	Colour& operator*=(const double factor);
	double& operator[](int i);  
	double operator[](int i) const; 
...
struct Texture {
	...
	Colour getColor(const Point3D& uvCoord);
...
struct Material {
	Material(Colour ambient, Colour diffuse, Colour specular, double exp, double idx, bool transmissive, Texture* texture) :
		ambient(ambient), diffuse(diffuse), specular(specular),
		specular_exp(exp), refrac_idx(idx), transmissive(transmissive), texture(texture) {}
	...
	Colour getTextureColor(const Ray3D& ray);
